	...
	$.fn.extend({
		...
		function addNewLightBoxItem(item,opt,direction) {
			...
			if (imgtyp=="video") {
				...
				lboxitem.append('<div class="tp-mainimage" style="width:'+ww+'px;height:'+hh+'px">'+imgsrc+'</div>');
			 } else {
			...
			var emailSrc = opt.emailUrlCustomPrefix+imgsrc+opt.emailUrlCustomSuffix;
			var emailbody = opt.emailBody.replace('#url',emailSrc);
			var email=$('<div class="email"><div class="social_tab"><a href="'+emailbody+'">'+opt.emailLinkText+'</a></div></div>');			
			...
			if (opt.showEmail=="yes") {
				soc.data('email',email);
				soc.append('<div class="twitter fakesoc"></div>');
			}
		...
		function lightBoxItemIn(dir) {		// DIR 1:  <-      DIR2: ->   //
					...
					setTimeout(function() {
							...
							if (opt.showEmail=="yes") soc.append(soc.data('email'));
